...
def get_jars_full(adir):
    ...
    ret = []
    for f in files:
        if f.endswith(".jar") and not (os.path.islink(os.path.join(adir,f)) and not os.path.exists(os.readlink(os.path.join(adir,f)))):
            ret.append(adir + "/" + f)
    return ret
...
def confvalue(name, extrapaths):
    global CONFFILE, JAVA_CMD
    command = [
        JAVA_CMD, "-client", get_config_opts(), "-Dstorm.conf.file=" +
                                               CONFFILE, "-cp", get_classpath(extrapaths), "backtype.storm.command.config_value", name
    ]
    p = sub.Popen(command, stdout=sub.PIPE)
    output, errors = p.communicate()
    lines = output.split("\n")
